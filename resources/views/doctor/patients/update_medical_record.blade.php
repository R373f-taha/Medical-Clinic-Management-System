...
<div style="background-color:#f3f3f3; min-height:100vh; padding:30px">
    ...
    <div style="max-width:800px; margin:auto; background:#fff; padding:30px; border-radius:8px;
        ...
        <form action="{{ route('doctor.medical_records.update',$medicalRecord) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div style="margin-bottom:15px;">
                ...
                <textarea name="diagnosis" rows="3"
                            style="width:100%; padding:10px; border:1px solid #ccc; border-radius:5px;">{{ $medicalRecord->diagnosis }}</textarea>
            </div>
            ...
            <div style="margin-bottom:15px;">
                ...
                <textarea name="notes" rows="3"
                            style="width:100%; padding:10px; border:1px solid #ccc; border-radius:5px;">{{ $medicalRecord->notes }}</textarea>
            </div>
            ...
            <div style="margin-bottom:15px;">
                ...
                <textarea name="treatment_plan" rows="3"
                            style="width:100%; padding:10px; border:1px solid #ccc; border-radius:5px;">{{ $medicalRecord->treatment_plan }}</textarea>
            </div>
            ...
            <div style="display:flex; gap:10px;">
                <button type="submit"
                        style="background-color:#ff7a00; color:#fff; padding:10px 18px;
                                border:none; border-radius:6px; cursor:pointer;">
                    Update
                </button>

                <a href="{{ route('doctor.medical_records.index') }}"
                    style="background-color:#6c757d; color:#fff; padding:10px 18px;
